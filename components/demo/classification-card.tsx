...
function splitSentences(text: string): string[] {
  // Only split at punctuation followed by space + Capital+lowercase
  // This avoids splitting on abbreviations like U.S.C., Art. 17, §2258A, etc.
  const parts = text.split(/(?<=[.!?])\s+(?=[A-Z][a-z])/);
...
function Truncated({
  ...
  const clipped = sentences.length > max;
  const shown = expanded ? text : sentences.slice(0, max).join(" ");
    ...
    <span className={className}>
      ...
      {clipped && (
        <button
          ...
          className="ml-1.5 text-blue-500 hover:text-blue-700 text-[11px] font-medium transition-colors"
        >
...
export default function ClassificationCard({ result, isLoading, streamingReasoning }: Props) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      if (sevBarRef.current)
        sevBarRef.current.style.width = `${result.severity * 10}%`;
      if (confBarRef.current)
        confBarRef.current.style.width = `${result.confidence}%`;
    }, 80);
    ...
    return (
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 sm:p-6">
        <div className="space-y-4">
          <div className="animate-pulse h-6 w-24 bg-slate-100 rounded-full" />
          <div className="animate-pulse h-8 w-3/4 bg-slate-100 rounded-lg" />
          <div className="animate-pulse h-14 bg-slate-100 rounded-lg" />
          ...
          {streamingReasoning ? (
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
              ...
              <p className="text-sm text-slate-700 leading-relaxed">
                {streamingReasoning}
                <span className="inline-block w-px h-[1em] bg-blue-500 animate-pulse ml-0.5 align-middle" />
              </p>
            </div>
          ) : (
          ...
          <div className="flex gap-2">
            {[1, 2, 3].map((i) => (
              <div key={i} className="animate-pulse h-6 w-20 bg-slate-100 rounded-full" />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 sm:p-6">
      <div className="flex items-start justify-between gap-4 mb-1">
        ...
        >
          <span
            className={`w-1.5 h-1.5 rounded-full animate-pulse ${risk.dot}`}
          />
          {result.risk_level.toUpperCase()}
        </div>
        <span className="font-mono text-[11px] text-slate-400 mt-1 shrink-0">
          {result._id}
        </span>
      </div>
      ...
      <p className="text-sm text-slate-500 leading-relaxed mb-5">
        <Truncated text={result.summary} max={2} />
      </p>
        ...
        {[
          {
            ...
            ref: sevBarRef,
            barClass: `bg-gradient-to-r ${risk.bar}`,
          },
          {
            label: "Confidence",
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-4">
        ...
        <p className="text-sm text-slate-700 leading-relaxed">
          <Truncated text={result.ai_reasoning} max={4} />
        </p>
      </div>
